...
Look through https://github.com/pyfar/haiopy/issues for bugs or feature request
...
Ready to contribute? Here's how to set up `haiopy` for local development using the command-line interface. Note that several alternative user interfaces exist, e.g., the Git GUI, `GitHub Desktop <https://desktop.github.com/>`_, extensions in `Visual Studio Code <https://code.visualstudio.com/>`_ ...

1. `Fork <https://docs.github.com/en/get-started/quickstart/fork-a-repo/>`_ the `haiopy` repo on GitHub.
2. Clone your fork locally and cd into the haiopy directory::

    $ git clone https://github.com/YOUR_USERNAME/haiopy.git
    $ cd haiopy

3. Install your local copy into a virtualenv. Assuming you have Anaconda or Miniconda installed, this is how you set up your fork for local development::

    $ conda create --name haiopy python
    $ conda activate haiopy
    $ conda install pip
    $ pip install -e .
...
5. When you're done making changes, check that your changes pass flake8 and the
   ...
   flake8 test must pass without any warnings for `./haiopy` and `./tests` using the default or a stricter configuration. Flake8 ignores `E123/E133, E226` and `E241/E242` by default. If necessary adjust the your flake8 and linting configuration in your IDE accordingly.
...
3. If checks to not pass, have a look at https://app.circleci.com/pipelines/github/pyfar/haiopy for more information.


Testing Guidelines
-----------------------
haiopy uses test-driven development based on `three steps <https://martinfowler.com/bliki/TestDrivenDevelopment.html>`_ and `continuous integration <https://en.wikipedia.org/wiki/Continuous_integration>`_ to test and monitor the code.
In the following, you'll find a guideline. Note: these instructions are not generally applicable outside of haiopy.

- The main tool used for testing is `pytest <https://docs.pytest.org/en/stable/index.html>`_.
- All tests are located in the *tests/* folder.
- Make sure that all important parts of haiopy are covered by the tests. This can be checked using *coverage* (see below).
- In case of haiopy, mainly **state verification** is applied in the tests. This means that the outcome of a function is compared to a desired value (``assert ...``). For more information, it is refered to `Martin Fowler's article <https://martinfowler.com/articles/mocksArentStubs.html.>`_.
